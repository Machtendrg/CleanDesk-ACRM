...
def process_csv_for_pass_fail_and_generate_pdfs(csv_path: str, endpoint: str, model: str, analysis_column: str, output_path: str, pdf_output_dir: str, console_output, generate_pdfs):
    """
    ...
    for index, row in data.iterrows():
        ...
        log_to_console(console_output, f"Querying LLM for record {index + 1} out of {len(data)}...")
        ...
        if ai_response:
            ...
            if compliant == "NO" and generate_pdfs:
            ...
            log_to_console(console_output, f"Response for record {index + 1}: {ai_response} (Compliant: {compliant})")
        else:
...
def run_ai_on_csv(input_csv, start_date, end_date, console_output, generate_pdfs):
    """
    ...
    try:
        ...
        # Make `filtered_data` available for saving through the GUI
        global filtered_data_global
        filtered_data_global = filtered_data.copy()
        ...
        process_csv_for_pass_fail_and_generate_pdfs(
            filtered_csv_path, endpoint, model, analysis_column, "output_with_ai.csv", pdf_output_dir, console_output, generate_pdfs
        )
...
def run_gui():
    ...
    def run_ai():
        ...
        run_ai_on_csv(input_csv, start_date, end_date, console_output, generate_pdfs)
